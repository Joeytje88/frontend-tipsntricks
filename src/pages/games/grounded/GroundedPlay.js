...
const GroundedPlay = () => {
    ...
    const handleClick = async () =>{
        try {
            const placecomment = await axios.post(`http://localhost:8080/api/post/34/comment/${userid}`,{
                text: inputComment,
    ...
    const getpost = async ()=> {
        try {
            const result = await axios.get(`http://localhost:8080/api/post/34`)
            setPost(result.data)
        } catch (error) {
            console.log(error)
        }
